...
def reset_daily_counter(user_id):
    today = date.today().isoformat()
    with sqlite3.connect('users.db') as conn:        c = conn.cursor()
...
def youmoney_webhook():
    try:
        ...
        # Проверка платежа        amount = data.get('amount')
        if (
...
def premium_info(message):
    if check_premium(message.from_user.id):
        ...
        return
    bot.reply_to(message,        f"⭐ **PREMIUM**\n\n"
...
if __name__ == '__main__':
    ...
    print("🚀 Bot started...")    bot.polling(none_stop=True)
